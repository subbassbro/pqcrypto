...
/*
 * Encoding formats (nnnn = log of degree, 8 for Falcon 256, 9 for Falcon-512, 10 for Falcon-1024)
 *
 *   private key:
...
/* see api.h */
int
PQCLEAN_FALCON256_CLEAN_crypto_sign_keypair(
    uint8_t *pk, uint8_t *sk) {
    union {
        uint8_t b[FALCON_KEYGEN_TEMP_8];
        uint64_t dummy_u64;
        fpr dummy_fpr;
    } tmp;
    int8_t f[256], g[256], F[256];
    uint16_t h[256];
    ...
    PQCLEAN_FALCON256_CLEAN_keygen(&rng, f, g, F, NULL, h, 8, tmp.b);
    ...
    sk[0] = 0x50 + 8;
    u = 1;
    v = PQCLEAN_FALCON256_CLEAN_trim_i8_encode(
            sk + u, PQCLEAN_FALCON256_CLEAN_CRYPTO_SECRETKEYBYTES - u,
            f, 8, PQCLEAN_FALCON256_CLEAN_max_fg_bits[8]);
    if (v == 0) {
        return -1;
    }
    u += v;
    v = PQCLEAN_FALCON256_CLEAN_trim_i8_encode(
            sk + u, PQCLEAN_FALCON256_CLEAN_CRYPTO_SECRETKEYBYTES - u,
            g, 8, PQCLEAN_FALCON256_CLEAN_max_fg_bits[8]);
    if (v == 0) {
        return -1;
    }
    u += v;
    ...
    u += v;
    if (u != PQCLEAN_FALCON256_CLEAN_CRYPTO_SECRETKEYBYTES) {
        return -1;
    }
     ...
     */
    pk[0] = 0x00 + 8;
    v = PQCLEAN_FALCON256_CLEAN_modq_encode(
            pk + 1, PQCLEAN_FALCON256_CLEAN_CRYPTO_PUBLICKEYBYTES - 1,
            h, 8);
    if (v != PQCLEAN_FALCON256_CLEAN_CRYPTO_PUBLICKEYBYTES - 1) {
        return -1;
    }
...
do_sign(uint8_t *nonce, uint8_t *sigbuf, size_t *sigbuflen,
    ...
    int8_t f[256], g[256], F[256], G[256];
    union {
        int16_t sig[256];
        uint16_t hm[256];
    } r;
    ...
    u = 1;
    v = PQCLEAN_FALCON256_CLEAN_trim_i8_decode(
            f, 8, PQCLEAN_FALCON256_CLEAN_max_fg_bits[8],
            sk + u, PQCLEAN_FALCON256_CLEAN_CRYPTO_SECRETKEYBYTES - u);
    if (v == 0) {
        return -1;
    ...
    if (!PQCLEAN_FALCON256_CLEAN_complete_private(G, f, g, F, 8, tmp.b)) {
        return -1;
    }
    ...
    PQCLEAN_FALCON256_CLEAN_hash_to_point_ct(&sc, r.hm, 8, tmp.b);
     ...
     */
    for (;;) {
        PQCLEAN_FALCON256_CLEAN_sign_dyn(r.sig, &sc, f, g, F, G, r.hm, 8, tmp.b);
        v = PQCLEAN_FALCON256_CLEAN_comp_encode(sigbuf, *sigbuflen, r.sig, 8);
        if (v != 0) {
            inner_shake256_ctx_release(&sc);
            *sigbuflen = v;
            return 0;
...
do_verify(
    ...
    if (PQCLEAN_FALCON256_CLEAN_modq_decode(h, 8,
                                            pk + 1, PQCLEAN_FALCON256_CLEAN_CRYPTO_PUBLICKEYBYTES - 1)
            != PQCLEAN_FALCON256_CLEAN_CRYPTO_PUBLICKEYBYTES - 1) {
        return -1;
    }
    PQCLEAN_FALCON256_CLEAN_to_ntt_monty(h, 8);
    ...
    if (PQCLEAN_FALCON256_CLEAN_comp_decode(sig, 8, sigbuf, sigbuflen) != sigbuflen) {
        return -1;
    }
    ...
    inner_shake256_inject(&sc, m, mlen);
    inner_shake256_flip(&sc);
    PQCLEAN_FALCON256_CLEAN_hash_to_point_ct(&sc, hm, 8, tmp.b);
    inner_shake256_ctx_release(&sc);

    /*
     * Verify signature.
     */
    if (!PQCLEAN_FALCON256_CLEAN_verify_raw(hm, sig, h, 8, tmp.b)) {
        return -1;
    }
    return 0;
}
...
/* see api.h */
int
PQCLEAN_FALCON256_CLEAN_crypto_sign_signature(
    uint8_t *sig, size_t *siglen,
    const uint8_t *m, size_t mlen, const uint8_t *sk) {
    /*
     * The PQCLEAN_FALCON256_CLEAN_CRYPTO_BYTES constant is used for
    ...
    vlen = PQCLEAN_FALCON256_CLEAN_CRYPTO_BYTES - NONCELEN - 3;
...
/* see api.h */
int
PQCLEAN_FALCON256_CLEAN_crypto_sign_verify(
...
/* see api.h */
int
PQCLEAN_FALCON256_CLEAN_crypto_sign(
    ...
    pm = sm + 2 + NONCELEN;
    sigbuf = pm + 1 + mlen;
    sigbuflen = PQCLEAN_FALCON256_CLEAN_CRYPTO_BYTES - NONCELEN - 3;
...
/* see api.h */
int
PQCLEAN_FALCON256_CLEAN_crypto_sign_open(
