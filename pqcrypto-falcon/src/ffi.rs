...
pub const PQCLEAN_FALCON256_CLEAN_CRYPTO_SECRETKEYBYTES: usize = 641;
pub const PQCLEAN_FALCON256_CLEAN_CRYPTO_PUBLICKEYBYTES: usize = 449;
pub const PQCLEAN_FALCON256_CLEAN_CRYPTO_BYTES: usize = 362; // I had to guess here, so don't take this number as correct
...
#[link(name = "falcon-256_clean")]
extern "C" {
    pub fn PQCLEAN_FALCON256_CLEAN_crypto_sign_keypair(pk: *mut u8, sk: *mut u8) -> c_int;
    pub fn PQCLEAN_FALCON256_CLEAN_crypto_sign(
        sm: *mut u8,
        smlen: *mut usize,
        msg: *const u8,
        len: usize,
    ...
    pub fn PQCLEAN_FALCON256_CLEAN_crypto_sign_open(
        m: *mut u8,
        mlen: *mut usize,
        sm: *const u8,
        smlen: usize,
    ...
    pub fn PQCLEAN_FALCON256_CLEAN_crypto_sign_signature(
        sig: *mut u8,
        siglen: *mut usize,
        m: *const u8,
        mlen: usize,
    ...
    pub fn PQCLEAN_FALCON256_CLEAN_crypto_sign_verify(
        sig: *const u8,
        siglen: usize,
        m: *const u8,
        mlen: usize,
...
mod test_falcon256_clean {
    ...
    fn test_ffi() {
        unsafe {
            ...
            let mut pk = vec![0u8; PQCLEAN_FALCON256_CLEAN_CRYPTO_PUBLICKEYBYTES];
            let mut sk = vec![0u8; PQCLEAN_FALCON256_CLEAN_CRYPTO_SECRETKEYBYTES];
            let mut pk_alt = vec![0u8; PQCLEAN_FALCON256_CLEAN_CRYPTO_PUBLICKEYBYTES];
            let mut sk_alt = vec![0u8; PQCLEAN_FALCON256_CLEAN_CRYPTO_SECRETKEYBYTES];
            let mut detached_sig = vec![0u8; PQCLEAN_FALCON256_CLEAN_CRYPTO_BYTES];
            let mut sm = Vec::with_capacity(mlen + PQCLEAN_FALCON256_CLEAN_CRYPTO_BYTES);
            let mut smlen = 0;
            assert_eq!(
                0,
                PQCLEAN_FALCON256_CLEAN_crypto_sign_keypair(pk.as_mut_ptr(), sk.as_mut_ptr())
            );
            assert_eq!(
                0,
            ...
            let mut unpacked_m = Vec::with_capacity(mlen + PQCLEAN_FALCON256_CLEAN_CRYPTO_BYTES);
            assert_eq!(
                0,
            ...
            assert!(
                smlen <= PQCLEAN_FALCON256_CLEAN_CRYPTO_BYTES,
